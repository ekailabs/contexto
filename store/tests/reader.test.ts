...
describe('EventReader', () => {
  ...
  describe('listAgents', () => {
    ...
    it('lists agent directories sorted lexically', () => {
      writer.append({ hook: 'session_start', agentId: 'beta', sessionId: 's1', event: {} });
      writer.append({ hook: 'session_start', agentId: 'alpha', sessionId: 's1', event: {} });
  ...
  describe('listSessions', () => {
    it('returns empty array for unknown agent', () => {
      expect(reader.listSessions('nonexistent')).toEqual([]);
    });

    it('lists sessions sorted by mtime (newest first)', async () => {
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 'old', event: {} });
      // Small delay to ensure different mtime
      await new Promise(r => setTimeout(r, 50));
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 'new', event: {} });

      const sessions = reader.listSessions('a1');
    ...
    it('sanitizes agentId input', () => {
      writer.append({ hook: 'session_start', agentId: 'my/agent', sessionId: 's1', event: {} });
      const sessions = reader.listSessions('my/agent');
  ...
  describe('readSession', () => {
    it('returns empty array for missing session', () => {
      expect(reader.readSession('a1', 's1')).toEqual([]);
    });

    it('reads all events from a session', () => {
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 's1', event: {} });
      writer.append({ hook: 'message_received', agentId: 'a1', sessionId: 's1', event: { text: 'hi' } });
      writer.append({ hook: 'session_end', agentId: 'a1', sessionId: 's1', event: {} });

      const events = reader.readSession('a1', 's1');
      expect(events).toHaveLength(3);
      expect(events.map(e => e.hook)).toEqual(['session_start', 'message_received', 'session_end']);
    });

    it('sanitizes input IDs', () => {
      writer.append({ hook: 'session_start', agentId: 'a/1', sessionId: 's/1', event: {} });
      const events = reader.readSession('a/1', 's/1');
    ...
    it('deduplicates by id when dedupe=true', () => {
      writer.append({ id: 'same-id', hook: 'message_received', agentId: 'a1', sessionId: 's1', event: { n: 1 } });
      writer.append({ id: 'same-id', hook: 'message_received', agentId: 'a1', sessionId: 's1', event: { n: 2 } });
      writer.append({ id: 'diff-id', hook: 'message_received', agentId: 'a1', sessionId: 's1', event: { n: 3 } });

      const raw = reader.readSession('a1', 's1');
      expect(raw).toHaveLength(3);

      const deduped = reader.readSession('a1', 's1', { dedupe: true });
  ...
  describe('reconstructSession', () => {
    it('reconstructs a basic session with user and assistant turns', () => {
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 's1', event: {} });
      writer.append({
    ...
    it('respects event.role over hook-name heuristic', () => {
      writer.append({
        hook: 'message_received', agentId: 'a1', sessionId: 's1',
    ...
    it('pairs tool calls by toolCallId', () => {
      writer.append({
    ...
    it('pairs tool calls by runId + toolName', () => {
      writer.append({
        hook: 'before_tool_call', agentId: 'a1', sessionId: 's1',
        ts: 1000,
    ...
    it('pairs tool calls by sequence (fallback)', () => {
      writer.append({
        hook: 'before_tool_call', agentId: 'a1', sessionId: 's1',
        ts: 1000,
    ...
    it('extracts tool calls from llm_output', () => {
      writer.append({
        hook: 'llm_output', agentId: 'a1', sessionId: 's1',
        event: {
    ...
    it('tracks userId attribution correctly', () => {
    ...
    it('marks unknown attribution when no userId available', () => {
      writer.append({
        hook: 'llm_output', agentId: 'a1', sessionId: 's1',
    ...
    it('extracts token usage from llm_output', () => {
      writer.append({
        hook: 'llm_output', agentId: 'a1', sessionId: 's1',
        event: {
          content: 'response',
    ...
    it('handles session_start and session_end timestamps', () => {
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 's1', ts: 1000, event: {} });
      writer.append({ hook: 'message_received', agentId: 'a1', sessionId: 's1', ts: 2000, event: { content: 'hi' } });
      writer.append({ hook: 'session_end', agentId: 'a1', sessionId: 's1', ts: 3000, event: {} });
    ...
    it('creates system turns for unrecognized hooks', () => {
      writer.append({ hook: 'before_prompt_build', agentId: 'a1', sessionId: 's1', event: {} });
      writer.append({ hook: 'agent_end', agentId: 'a1', sessionId: 's1', event: {} });
    ...
    it('reconstructs in eventTs order, not write order', () => {
    ...
    it('does not double-match a before_tool_call with multiple indexes', () => {
  ...
  describe('ID flow: listAgents → listSessions → readSession', () => {
    it('list output is directly usable as input to downstream methods', () => {
      writer.append({ hook: 'session_start', agentId: 'my-agent', sessionId: 'sess-1', event: {} });
      writer.append({ hook: 'message_received', agentId: 'my-agent', sessionId: 'sess-1', event: { content: 'hi' } });

      const agents = reader.listAgents();
      expect(agents).toHaveLength(1);

      const sessions = reader.listSessions(agents[0]);
      expect(sessions).toHaveLength(1);

      const events = reader.readSession(agents[0], sessions[0]);
      expect(events).toHaveLength(2);

      const session = reader.reconstructSession(agents[0], sessions[0]);
      expect(session.turns).toHaveLength(1);
      expect(session.turns[0].content).toBe('hi');
    });

    it('raw IDs still work as input (backward compat)', () => {
      writer.append({ hook: 'session_start', agentId: 'raw-agent', sessionId: 'raw-sess', event: {} });

      const sessions = reader.listSessions('raw-agent');
      expect(sessions).toHaveLength(1);

      const events = reader.readSession('raw-agent', 'raw-sess');
      expect(events).toHaveLength(1);
    });

    it('rejects path-traversal attempts in agentId', () => {
      writer.append({ hook: 'session_start', agentId: 'legit', sessionId: 's1', event: {} });

      // '..' would resolve to parent of dataDir — must not be trusted
      const events = reader.readSession('..', 's1');
    ...
    it('rejects path-traversal attempts in sessionId', () => {
      writer.append({ hook: 'session_start', agentId: 'a1', sessionId: 's1', event: {} });

      const events = reader.readSession('a1', '../../../etc/passwd');
      expect(events).toEqual([]);
    });
  });
});
