...
export abstract class AbstractContextEngine implements ContextEngine {
  ...
  async assemble(params: AssembleParams): Promise<AssembleResult> {
    ...
    this.logger.info(`[contexto] assemble() called — ${messages?.length} messages, tokenBudget: ${tokenBudget}, hasApiKey: ${!!this.config.apiKey}`);
    ...
    if (!this.config.apiKey) {
      this.logger.info(`[contexto] assemble() skipping — no apiKey`);
