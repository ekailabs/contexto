...
if [ "$ENABLE_OPENROUTER" != "false" ] && [ "$ENABLE_OPENROUTER" != "0" ]; then
  ...
  cd /app/packages/openrouter
  OPENROUTER_PORT="$OPENROUTER_PORT" MEMORY_DB_PATH="${MEMORY_DB_PATH:-/app/packages/memory/data/memory.db}" node dist/server.js &
  PIDS+=($!)
fi
